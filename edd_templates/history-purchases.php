		...
		<?php foreach ( $purchases as $post ) : setup_postdata( $post ); ?>
			...
			<tr class="edd_purchase_row">
				...
				<td class="edd_purchase_details">
					...
					<a class="btn btn-success" href="<?php echo add_query_arg( 'payment_key', edd_get_payment_key( $post->ID ), edd_get_success_page_uri() ); ?>">&raquo;</a>
					<?php else: ?>
					<a class="btn btn-success" href="<?php echo add_query_arg( 'payment_key', edd_get_payment_key( $post->ID ), edd_get_success_page_uri() ); ?>"><span class="fa fa-download"></span><?php _e( 'View Details and Downloads', 'edd' ); ?></a>
					<?php endif; ?>
				</td>
